...
def test_audio_volume_modification(client, mocker):
    ...
    response = client.put(
            url=f"/audio/{filename}/adjust_volume", headers={"x-api-key": "TEST_KEY"}, json={"volume_increase": 1}
        )
    
    assert response.status_code == HTTPStatus.NO_CONTENT
    assert mocked_function.called_once_with(os.path.join(settings.STORAGE_PATH, filename))
